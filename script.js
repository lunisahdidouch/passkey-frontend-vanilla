...
btnRegister.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!await handleFetchError(respOptions, "fetching registration options")) return;
        ...
        if (!await handleFetchError(verificationResp, "sending registration to server")) return;
...
btnLogin.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!await handleFetchError(respOptions, "fetching authentication options")) return;
        const optionsFromServer = await respOptions.json();
        ...
        if (!await handleFetchError(verificationResp, "sending authentication to server")) return;
        const verificationJSON = await verificationResp.json();
